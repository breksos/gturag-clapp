...
- Derivation: `scripts/emblem.py` traces those three flat colours into paths. It is a
  ...
  (docs/ICONS.md rule 5: *use the real mark, and don't design one yourself*). The wordmark
  is dropped; only the emblem is used.
...
Teknik Üniversitesi.** It is an independent tool that searches the forms the university
publishes; the emblem identifies whose forms these are, and claims nothing else. The
university's rights in its own mark are unaffected by this use, and it can be removed on
request — `scripts/emblem.py` and `src/Emblem.tsx` are the only two places it lives.

The interface's three accent colours are sampled from that same emblem: navy `#1a3476`,
crimson `#cd1239`, orange `#f58612`.
